...
def test_tagger():
    ...
    manager = FullStorageManager(CustomTagger(FixedCameraBGFGSegmenter().segment, batch_size=20), CropSplitter(), '/tmp/videos', '/tmp/videos', dsn='dbname=postgres user=postgres password=postgres host=127.0.0.1')
    start = time.time()
