...
class FullStorageManager(StorageManager):
    ...
    def put(self, filename, target, args=DEFAULT_ARGS, in_extern_storage=False, parallel=False, rows=None, hwang=False, do_join=True):
        ...
        if parallel and not stream:
            ...
            write_video_single(conn, filename, target, physical_dir, self.content_splitter, tagger, stream=stream, args=args, background_scale=args['background_scale'], rows=rows, hwang=hwang, do_join=True)
