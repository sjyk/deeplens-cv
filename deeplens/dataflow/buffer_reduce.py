...
class BufferReduce(Operator):
    ...
    def __init__(self,
        ...
        the size of the buffer, a sampling rate, a resolution resizing and
        two more for buffer size tuning - the alpha in EWMA and a slow-start threshold.
        """
    ...
    # this is the main routine which buffers up the frames and applies a reduce function
    # to each buffer
    def _reduce(self):
    ...
    def reduce(self, data):
        """reduce() must be implemented by each subclass.

            Args:
                data - list of raw frames

            Returns:
                output - dict of reduce outputs
        """
        raise NotImplemented("BufferReduce must implement a reduce function")
    ...
    def __next__(self):
        ...
        if self.buffer_state == 0:
            # buffer is filled so run reduce first time
...
class TestBufferReduce(BufferReduce):
    ...
    def reduce(self, data):
        print('[Buffer Reduce] Processed: ',len(data), 'frames')
        return {'number of frames': len(data)}
